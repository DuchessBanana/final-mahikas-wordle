...
public class Display {
    ...
    private int x, y, w, h, x2, y2, w2, h2; // (x, y) of upper left corner of display
    // the width and height of the display

    private float dx, dy, dx2, dy2; // calculate the width and height of each box
    ...
    public Display(PApplet p, int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2) {
        this.x = x1;
        this.y = y1;
        this.w = w1;
        this.h = h1;
    ...
    public void drawKeys(int[][] f) {
        ...
        for (int i = 0; i < numrows; i++) {
            for (int j = 0; j < numcols; j++) {
                ...
                if (pieceImage != null) {
                    p.image(pieceImage, x2 + j * dx2, y2 + i * dy2, dx2, dy2);
                } else {
                    p.fill(getColor(piece));
                    p.rect(x2 + j * dx2, y2 + i * dy2, dx2, dy2);
                }
            }
        }
    }
    ...
    public void displayKeysOnGrid(String[][] grid, int fontsize, int horizontal_adjust, int vertical_adjust) {
        String text;
        int numcols = grid[0].length;
        int numrows = grid.length;
        ...
        for (int i = 0; i < numrows; i++) {
            for (int j = 0; j < numcols; j++) {
                text = grid[i][j];
                p.fill(p.color(0));   // display text in black
                p.textMode(p.CENTER);
                p.text(text, x2 + j * dx2 + dx2 / 2 - fontsize / 2 + horizontal_adjust,
                        y2 + i * dy2 + dy2 / 2 + fontsize / 2 + vertical_adjust);
            }
        }
    }
    ...
    public void setKeyboardCols(int numCols) {
        dx2 = w2 / numCols;
    }

    public void setKeyboardRows(int numRows) {
        dy2 = h2 / numRows;
    }

    public void initializeWithGame(GameBoard game, Keyboard keyboard) {
        int[][] grid = game.getGrid();
        int[][] keyGrid = keyboard.getGrid();
        if (grid == null) {
            System.out
        ...
        setKeyboardCols(keyGrid[0].length);
        setKeyboardRows(keyGrid.length);
    }
}